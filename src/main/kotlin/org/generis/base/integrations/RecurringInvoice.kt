...
class RecurringInvoice {
    ...
    fun invoiceMail(invoice: Invoice): String {
        return """
    <!DOCTYPE html>
    <html lang="en">
      <head>
      ...
      <body>
        <div class="invoice">
          ...
          <div class="invoice-details">
            ...
            <p>Dear ${invoice.customerId?.name ?: invoice.company?.name ?: "valued customer"}, here is a breakdown of your invoice</p>
          </div>
          <table class="invoice-table">
            <thead>
              <tr>
            ...
            <tfoot>
              <tr>
                ...
                <td>${invoice.customerId?.country?.currencyCode ?: invoice.company?.country?.currencyCode }${invoice.totalAmount}</td>
              </tr>
    ...
    private fun invoiceRows(invoice: Invoice): String {
        ...
        for (item in invoice.items!!) {
            ...
            <tr>
              <td>${item.productId?.productName}</td>
              <td>${item.quantity}</td>
              <td>${invoice.customerId?.country?.currencyCode ?: invoice.company?.country?.currencyCode }${item.productId?.unitPrice}</td>
              <td>${discount}</td>
              <td>${invoice.customerId?.country?.currencyCode ?: invoice.company?.country?.currencyCode}${item.totalAmount}</td>
            </tr>
        """
        }
        return tableRows
